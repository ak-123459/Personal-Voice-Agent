...
class PyPiper():
    ...
    def load_mod(self, instr="en_US-bryce-medium"):
        t0 = time.time()
        self.model = instr
        ...
        style = instr.split("-")[2]
        file = f'{instr}.onnx'
        model_path = os.path.join(os.getcwd(), 'voices', file)
        ...
        if not os.path.isfile(model_path):
            ...
            print(f"[load_mod] JSON config downloaded in {time.time()-t1:.2f}s ({len(json_file.content)} bytes)")

            t1 = time.time()
            mod_file = requests.get(m_path)
            print(f"[load_mod] Model downloaded in {time.time()-t1:.2f}s ({len(mod_file.content)} bytes)")

            with open(model_path, 'wb') as m:
                m.write(mod_file.content)
            with open(f'{model_path}.json', 'wb') as j:
                j.write(json_file.content)
        else:
            print(f"[load_mod] Found locally: {model_path}")

        self.json_ob = f'{model_path}.json'
