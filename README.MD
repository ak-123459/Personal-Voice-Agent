...
- 🎤 **Real-time Voice Input** - Record and transcribe speech with automatic processing
- 🔊 **Text-to-Speech Output** - AI responses converted to natural speech
- 🧠 **Powered by Groq LLM** - Fast inference using Llama 3.3 70B model
- ⚡ **WebSocket Communication** - Real-time bidirectional communication
- 🎵 **Music Playback** - Search and play music via YouTube
...
⚠️ **Important:**  
This project currently works **only in Google Chrome**.

### Reason
The application uses microphone access (`getUserMedia`) and relies on Chrome-specific handling of microphone permissions.  
Other browsers (such as Firefox, Edge, or Safari) may block or fail to grant microphone access due to different permission policies.

### Recommendation
Please open this project in **Google Chrome** and allow microphone permissions when prompted.

### Future Improvements
Cross-browser compatibility will be addressed in future updates.
...
<details>
<summary><b>❌ Error: <code>name 'tempfile' is not defined</code></b></summary>

**Solution:** Add import at top of `server.py`:
```python
import tempfile
```
</details>
...
<summary><b>🎤 Audio Not Recognized</b></summary>
...
<details>
<summary><b>🔇 No Audio Response</b></summary>
...
Enable detailed logging in `server.py` by checking console output:
...
```bash
# Test reminder
python -c "from server import set_reminder; print(set_reminder('Test', 5))"

# Test time
python -c "from server import get_current_time; print(get_current_time())"

# Test message
python -c "from server import send_message; print(send_message('Hello'))"
```
...
Replace Google Speech Recognition with Groq Whisper in `server.py`:
...
Add new functions to `tools` array and `function_map` in `server.py`:
...
- 🐛 **Bug Reports:** [Open an issue](https://github.com/ak-123459/Personal-Voice-Agent/issues)
- 💡 **Feature Requests:** [Open an issue](https://github.com/ak-123459/Personal-Voice-Agent/issues)
- 💬 **Discussions:** [GitHub Discussions](https://github.com/ak-123459/Personal-Voice-Agent/discussions)
...
Made with ❤️ by [Your Name](https://github.com/ak-123459)
